...
- Persist native exact high-bucket progress in browser local storage. Timed-out chunks advance only through fully completed discard candidates; the current candidate is retried on the next run so the proof cache remains conservative.
