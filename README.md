...
For maximization, the saved "best known" score is a lower bound on the true optimum. Browser-local records are stored by canonical deal key, so equivalent deals can reuse the strongest known lower bound with the placement translated onto the selected cards. The native high-bucket pass also stores safe resume progress by canonical deal key. Progress is tracked by fully checked discard candidates plus completed row partitions inside the current discard candidate, so timed-out chunks can resume within a long-running discard instead of restarting it. The exact passes cover the full proof space in phases: 8/9/10 hands, then 3+/4-row low buckets, then 0/1/2-row low buckets. The app still labels a result "Best Found" whenever those proof phases time out, so the remaining V0 work is making the exact proof path fast enough to exhaust reliably for arbitrary user-selected deals.
